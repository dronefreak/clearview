...
D="/home/neural_debugger/Downloads/datasets/clearview"
...
clearview-evaluate --model unet --weights "$WEIGHTS" \
  --data-dir "$D/archive/SPA-Data_6385/val" --dataset-type spa-data \
  --output-dir "$OUT/SPA-Data_val" --device cuda

# Rain13K's 5 benchmarks
for SET in Rain100L Rain100H Test100 Test1200 Test2800; do
  clearview-evaluate --model unet --weights "$WEIGHTS" \
    --data-dir "$D/some_rainy_dataset/test-20260803T113945Z-1-001/test/$SET" \
...
clearview-evaluate --model unet --weights "$WEIGHTS" \
  --data-dir "$D/rainy_image_dataset" \
  --dataset-type rain1400 --rainy-dir testing/rainy_image --clean-dir testing/ground_truth \
  --output-dir "$OUT/DDN_Data" --device cuda

# RealRain-1k-H / -L held-out test splits (224 pairs each)
clearview-evaluate --model unet --weights "$WEIGHTS" \
  --data-dir "$D/RealRain-1k/RealRain-1k/RealRain-1k-H/test" \
  --dataset-type pair --rainy-dir input --clean-dir target \
  --output-dir "$OUT/RealRain1k_H_test" --device cuda

clearview-evaluate --model unet --weights "$WEIGHTS" \
  --data-dir "$D/RealRain-1k/RealRain-1k/RealRain-1k-L/test" \
  --dataset-type pair --rainy-dir input --clean-dir target \
...
clearview-evaluate --model unet --weights "$WEIGHTS" \
  --data-dir "$D/AllWeather_Testing/rain" \
